...
let getWalletInfo = async function (apiKey) {
    try {
        ...
        let result = await send_get_request(URL_BASE + '/v1/wallet/owner/retrieve_summary_info')
...
let getTransactions = async function (apiKey) {
    try {
	    let URL_BASE = 'http://grin:' + apiKey + '@127.0.0.1:3420'
        let result = await send_get_request(URL_BASE + "/v1/wallet/owner/retrieve_txs?refresh")
...
let sendToAddress = async function (apiKey,address,amount) {
    ...
    try {
        ...
        let URL_BASE = 'http://grin:' + apiKey + '@127.0.0.1:3420'
        let url = URL_BASE+"/v1/wallet/owner/issue_send_tx"
...
let rePostTx = async function (apiKey, id) {
    try {
        let URL_BASE = 'http://grin:' + apiKey + '@127.0.0.1:3420'
        let url = URL_BASE+"/v1/wallet/owner/repost?fluff&tx_id?"+id
        console.log("url: ",url)
